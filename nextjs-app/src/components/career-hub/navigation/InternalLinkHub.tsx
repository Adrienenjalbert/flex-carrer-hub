...
import { getPersonaHub } from '@/lib/data/persona-hubs';
...
const _getHowToGuideLinks = (industry?: string, roleSlug?: string, limit: number = 6): LinkItem[] => {
...
// Get financial article links
const getFinancialArticleLinks = (articleSlugs: string[], excludeSlug?: string, limit: number = 6): LinkItem[] => {
  const links: (LinkItem | null)[] = articleSlugs
    .filter(slug => slug !== excludeSlug)
    .slice(0, limit)
    .map(slug => {
      ...
      if (!article) return null;
      return {
        href: `/career-hub/financial-tips/${slug}`,
        label: article.title,
        description: article.readTime,
      };
    });
...
const getContextualRecommendations = (context?: PageContext): ContextualRecommendations => {
  ...
  if (context.type === 'tool' && context.slug) {
    ...
    const financialLinks = getFinancialArticleLinks(financialGuides, undefined, 3);
  ...
  if (context.type === 'application') {
    const sections: LinkSection[] = [
      ...
      },
      {
        ...
        links: [
          { href: '/career-hub/guides/first-flex-job', label: 'Finding Flexible Work', description: 'Your options explained' },
        ],
      },
    ];
...
export const InternalLinkHub = ({
  variant = 'sidebar',
  currentPage,
  maxLinksPerSection: _maxLinksPerSection = 6,
